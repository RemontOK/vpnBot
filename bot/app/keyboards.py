...
def plans_keyboard(plans: list[dict]) -> InlineKeyboardMarkup:
    ...
    for plan in plans:
        ...
        rows.append(
            [InlineKeyboardButton(text=text, callback_data=f"buy:{plan['id']}")]
        )
    rows.append(
...
def protocol_keyboard(plan_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="VLESS", callback_data=f"buy:{plan_id}:vless"
                ),
                InlineKeyboardButton(
                    text="Hysteria", callback_data=f"buy:{plan_id}:hysteria"
                ),
            ],
...
def apps_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="Android/iPhone/PC: Hiddify", url=HIDDIFY_DOWNLOAD_URL
                )
            ],
            [
                InlineKeyboardButton(
                    text="Google Play: Hiddify", url=HIDDIFY_ANDROID_URL
                )
            ],
...
def checkout_keyboard(
    order_id: str, confirmation_url: str | None
) -> InlineKeyboardMarkup:
...
def profile_actions_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔄 Обновить ссылку", callback_data="profile:refresh"
                )
            ],
            [InlineKeyboardButton(text="⬅ В главное меню", callback_data="menu:main")],
        ]
